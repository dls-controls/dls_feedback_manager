...
class XBPM_DCMFeedback:
    ...
    def __init__(self, XBPMSharedPVs, DCM_prefix):
        self.XBPMSharedPVs = XBPMSharedPVs
        self.prefix = DCM_prefix
...
class XBPM_FSWTfeedback(XBPM_DCMFeedback):
    ...
    def __init__(self, XBPMSharedPVs, FSWT_prefix, DCM_prefix): # Remove DCM_prefix
        XBPM_DCMFeedback.__init__(self, XBPMSharedPVs, DCM_prefix) # Same here - don't want this class to use other prefix
        self.XBPMSharedPVs = XBPMSharedPVs
    ...
    def enable_status_checker(self):
        if self.XBPMSharedPVs.XBPM_DCMFeedback.fb_enable_status.get() == 0:
            assert (self.XBPMSharedPVs.fb_enable_status.get() == 0, "Both XBPM1 and XBPM2 are off")
        elif self.XBPMSharedPVs.fb_enable_status.get() == 1:
            assert (self.XBPMSharedPVs.XBPM_FSWTFeedback.fb_enable_status.get() == 0, "Both XBPM1 and XBPM2 are on") 
    """
